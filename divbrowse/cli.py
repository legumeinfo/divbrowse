...
def get_config_skeleton():
    """Get the Divbrowse config file skeleton"""
    path_config_skeleton = os.path.join(
        os.path.dirname(__file__), "divbrowse.config.yml.skeleton"
    )
    try:
...
def get_chromosomes(path_zarr):
    """Get the chromosomes from a Zarr archive"""
...
@click.command()
def calcsumstats():
    """Calculate variant summary statistics"""
...
@click.option(
    "--path-vcf",
    help="Full path to to VCF file that should be converted to a Zarr archive",
)
@click.option("--path-zarr", help="Full path where to save the Zarr archive")
def vcf2zarr(path_vcf: str, path_zarr: str):
    """Convert a VCF file to a Zarr archive"""
    if path_vcf == None:
        vcf_files = []
        path = os.getcwd()
        ...
        click.secho(
            "The following VCF files have been found in current working directory. Please choose the one you want to convert to the Zarr format:",
            fg="yellow",
        )
        i = 0
        ...
        selected_vcf_number = click.prompt(
            click.style("Please enter the number of the VCF file", fg="yellow"),
            type=int,
        )
        ...
        click.secho(
            "You have selected the following VCF file:: " + path_vcf, fg="yellow"
        )
...
@click.option(
    "--host",
    default="0.0.0.0",
    help="IP address to bind the DivBrowse server to",
    show_default=True,
)
@click.option(
    "--port",
    default="8080",
    help="Port number to bind the DivBrowse server to",
    show_default=True,
)
@click.option(
    "--infer-config",
    is_flag=True,
    help="If set: infer a basic configuration from the provided VCF and GFF/GFF3 files and do not use an existing `divbrowse.config.yml`",
)
@click.option(
    "--save-config",
    type=click.Path(file_okay=True, writable=True),
    help="Save the inferred configuration as a YAML file. Please provide a relative or absolute path.",
)
def start(host: str, port: str, infer_config: bool, save_config):
    """Start the DivBrowse server"""
    ...
    if infer_config:
        ...
        click.secho(
            "The following VCF files have been found. Please choose the one you want to visualize with DivBrowse:",
            fg="yellow",
        )
        i = 0
        for _vcf_file in vcf_files:
        ...
        click.secho(
            "Your VCF choice: " + str(vcf_files[selected_vcf_number]), fg="yellow"
        )
        ...
        click.secho(
            "The following GFF/GFF3 files have been found. Please choose the one you want to visualize with DivBrowse:",
            fg="yellow",
        )
        i = 0
        ...
        selected_gff3_number = click.prompt(
            click.style("Please enter the number of the GFF/GFF3 file", fg="yellow"),
            type=int,
        )
        ...
        click.secho(
            "Your GFF/GFF3 choice: " + str(gff3_files[selected_gff3_number]),
            fg="yellow",
        )
    ...
    click.secho(
        "DivBrowse should be available under the following URL: " + str(url),
        fg="green",
        bold=True,
    )
