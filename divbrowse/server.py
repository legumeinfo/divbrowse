...
def create_app(filename_config_yaml="divbrowse.config.yml", config_runtime=None):
    ...
    if config_runtime is not None:
        ...
        try:
            ...
            log.error(
                "Divbrowse config file `divbrowse.config.yml` not found in current directory!"
            )
            exit(1)
    ...
    if brapi_active:
        ...
        app.register_blueprint(
            get_brapi_blueprint(config, gd, ad), url_prefix="/brapi/v2"
        )

    def process_request_vars(vars):
        """Process the request variables and return a dictionary with the processed values"""
        ...
        try:
            ...
            if "samples" in vars:
                ...
                samples, unmapable_sample_ids = (
                    gd.map_input_sample_ids_to_vcf_sample_ids(vars["samples"])
                )

                if len(unmapable_sample_ids) > 0:
                    raise ApiError(
                        "The following sample-IDs could not be resolved: "
                        + ", ".join(unmapable_sample_ids)
                    )
            ...
            if "variant_filter_settings" in vars:
                ...
                if type(vars["variant_filter_settings"]) is str:
                    vars["variant_filter_settings"] = json.loads(
                        vars["variant_filter_settings"]
                    )
    ...
    def __genomic_window_summary():
        """Return a summary of the genomic window"""
    ...
    def __clustermap():
        """Return a clustermap of the distance matrix"""
        payload = request.get_json(silent=True)
        ...
        clustergrid = sns.clustermap(
            distances, figsize=(20, 20), cmap=cmap, xticklabels=False, yticklabels=False
        )
        buffer = BytesIO()
        clustergrid.savefig(buffer, format="png")
        data = base64.b64encode(buffer.getbuffer()).decode("ascii")

        result = {"clustermap": data}

        return jsonify(result)

    @app.route("/variant_calls", methods=["GET", "POST", "OPTIONS"])
    def __variant_calls():
        """Return the variant calls for a given genomic window"""
        if request.method == "POST":
            input = process_request_vars(request.get_json(silent=True))
        else:
            ...
            return jsonify(
                {
                    "success": False,
                    "status": "error",
                    "message": "The provided chromosome number "
                    + str(input["chrom"])
                    + " is not included in the variant matrix.",
                }
            )
        ...
        if slice.sliced_variant_calls.ndim == 2:
            slice.sliced_variant_calls = (
                slice.sliced_variant_calls.T
            )  # transpose GenotypeArray so that samples are in the 1st dimension and not the variant-calls

        if slice.sliced_variant_calls.ndim == 3:
            slice.sliced_variant_calls = slice.sliced_variant_calls.transpose(
                1, 0, 2
            )  # transpose GenotypeArray so that samples are in the 1st dimension and not the variant-calls

        result = {
            "calls": dict(
                zip(slice.samples_selected_mapped, slice.sliced_variant_calls.tolist())
            ),
    ...
    def __variants():
        """Return the variants for a given genomic window"""
        start_all = timer()
        ...
        log.debug(
            "==== gd.get_slice_of_variant_calls() => calculation time: %f",
            timer() - start,
        )
        ...
        log.debug(
            "==== Analysis() + analysis.calc_distance_to_reference() => calculation time: %f",
            timer() - start,
        )
        ...
        if "QUAL" in gd.available_variants_metadata:
            sliced_qual = gd.variants_qual.get_basic_selection(
                slice.slice_variant_calls
            )
        ...
        if gd.available["snpeff"]:
            sliced_ann = gd.callset["variants/ANN"].get_basic_selection(
                slice.slice_variant_calls
            )
        ...
        if ad.available["gff3"]:
            ...
            start = timer()
            genes_within_slice = ad.genes.loc[
                (ad.genes["start"] <= curr_start) & (ad.genes["end"] >= curr_end)
            ]
            genes_starting_in_slice = ad.genes.loc[
                (ad.genes["start"] >= curr_start) & (ad.genes["start"] <= curr_end)
            ]
            genes_ending_in_slice = ad.genes.loc[
                (ad.genes["end"] >= curr_start) & (ad.genes["end"] <= curr_end)
            ]
            genes_all_in_slice = (
                pd.concat(
                    [genes_within_slice, genes_starting_in_slice, genes_ending_in_slice]
                )
                .drop_duplicates()
                .reset_index(drop=True)
            )
            genes_all_in_slice = genes_all_in_slice.loc[
                (genes_all_in_slice["seqid"] == ad.chrom_gff3_map[input["chrom"]])
            ]
            result["features"] = genes_all_in_slice.to_dict(orient="records")

            #### Nearest gene ##############################
            nearest_gene = ad.get_nearest_gene_start_pos(
                input["chrom"], int(gd.pos[slice.location_start])
            )
            result["nearest_feature"] = nearest_gene.to_dict(orient="records")

            log.debug(
                "==== time for gff3 + ad.get_nearest_gene_start_pos() section: %f",
                timer() - start,
            )
        ...
        response = Response(  # this is required because NaN is not valid JSON, so this dumps NaN to null
            response=json.dumps(result, ignore_nan=True),
            status=200,
    ...
    def __vcf_export_check():
        """Check if the requested genomic window is too big for VCF export"""
        if request.method == "POST":
            input = process_request_vars(request.get_json(silent=True))
        else:
            # raise ApiError('Method not allowed', status_code=405)
    ...
    def __vcf_export():
        """Return a VCF file for a given genomic window"""
        if request.method == "POST":
        ...
        if vcf_lines_header == None:
            ...
            if "DP" in gd.available_calldata:
                vcf_lines_header.append(
                    '##FORMAT=<ID=DP,Number=.,Type=Integer,Description="Read depth">'
                )

        vcf_columns = {"FORMAT": ["GT"]}

        # check for DP values and add to FORMAT column
        if "DP" in gd.available_calldata:
            vcf_columns["FORMAT"].append("DP")

        mapped_sample_ids, _ = gd.map_vcf_sample_ids_to_input_sample_ids(
            gd.samples[slice.samples_mask].astype(str).tolist()
        )
        vcf_line_variants_header = [
            "#CHROM",
            "POS",
            "ID",
            "REF",
        ...
        ref = gd.reference_allele.get_orthogonal_selection(
            (slice.filtered_positions_indices)
        )
        alts = gd.alternate_alleles.get_orthogonal_selection(
            (slice.filtered_positions_indices)
        )
        qual = gd.callset["variants/QUAL"].get_orthogonal_selection(
            (slice.filtered_positions_indices)
        )
        # mq = callset['variants/MQ'].get_orthogonal_selection( (slice.filtered_positions_indices) )

        def __generate():
            """Generator function to yield the VCF lines"""
            ...
            for pos_idx in slice.filtered_positions_indices.tolist():
                ...
                vcf_line = [
                    ...
                    str(ref[i]),
                    ",".join(
                        [_alt for _alt in (alts[i].astype(str).tolist()) if _alt != ""]
                    ),  #','.join([ _alt for _alt in (alts[i].astype(str).tolist()) if _alt != '' ]),
                    str(qual[i]),
                    "NA",
                    "",  #'MQ='+str(mq[i]),
                ...
                if "samples" in input:
                    ...
                    gt_slice = gd.callset["calldata/GT"].get_orthogonal_selection(
                        ([pos_idx], slice.samples_mask)
                    )

                    if "DP" in vcf_columns["FORMAT"]:
                        dp_slice = gd.callset["calldata/DP"].get_orthogonal_selection(
                            ([pos_idx], slice.samples_mask)
                        )

                else:
                    gt_slice = gd.callset["calldata/GT"].get_orthogonal_selection(
                        ([pos_idx], slice(None), slice(None))
                    )
        ...
        return Response(
            __generate(),
            mimetype="text/csv",
            headers={"Content-Disposition": "attachment; filename=custom_export.vcf"},
        )

    @app.route("/csv_export", methods=["GET", "POST", "OPTIONS"])
    def __csv_export():
        """Return a CSV file for a given genomic window"""
        if request.method == "POST":
            input = process_request_vars(request.form.to_dict())
        else:
            # raise ApiError('Method not allowed', status_code=405)
        ...
        csv_line_chroms = ["CHROM"] + [_chrom] * slice.filtered_positions_indices.shape[
            0
        ]
        ...
        def get_nucleotide(num_of_alternate_alleles, i):
            """Return the nucleotide for the given number of alternate alleles"""
            nuc = ""
        ...
        def __generate():
            """Generator function to yield the CSV lines"""
            yield "\n".join(csv_lines) + "\n"

            i = 0
            for sample_id in mapped_sample_ids:
                calls_for_sample = (
                    slice.numbers_of_alternate_alleles[i].astype(str).tolist()
                )
                nucleotides_for_sample = [
                    get_nucleotide(num_alt, i)
                    for i, num_alt in enumerate(calls_for_sample)
                ]
    ...
    def __gff3_export():
        """Return a GFF3 file for a given genomic window"""
            ...
            return jsonify(
                {
                    ...
                    "message": "The provided chromosome number "
                    + str(input["chrom"])
                    + " is not included in the SNP matrix.",
                }
            )
        ...
        def __generate():
            ...
            for index, row in genes_all_in_slice.iterrows():
                ...
                if row["primary_confidence_class"] != ".":
                    gff3_attributes.append(
                        "primary_confidence_class="
                        + str(row["primary_confidence_class"])
                    )
        ...
        return Response(
            __generate(),
            mimetype="text/csv",
            headers={"Content-Disposition": "attachment; filename=custom_export.gff3"},
        )

    @app.route("/blast", methods=["GET", "POST", "OPTIONS"])
    def __blast():
        """Run a BLAST search on the provided query sequence"""
        ...
        paste_content_result = gi.tools.paste_content(
            blast_parameters["query"], history_id, file_name="blast_query.fasta"
        )
        paste_content_dataset_id = paste_content_result["outputs"][0]["id"]

        tool_inputs = (
            inputs()
            .set_dataset_param("query", paste_content_dataset_id, src="hda")
        ...
        run_tool_result = gi.tools.run_tool(
            history_id, blast_parameters["galaxy_tool_id"], tool_inputs
        )
        result_dataset_id = run_tool_result["outputs"][0]["id"]

        blast_chromosome_mapping = config["blast"][
        ...
        gi.histories.delete_dataset(
            history_id=history_id, dataset_id=paste_content_dataset_id, purge=True
        )
        gi.histories.delete_dataset(
            history_id=history_id, dataset_id=result_dataset_id, purge=True
        )
        ...
        for line in blast_result_lines:
            if line != "":
                ...
                _single_blast_hit = {
                    ...
                    "snp_count": gd.count_variants_in_window(
                        _chromosome_vcf, int(line_parts[8]), int(line_parts[9])
                    ),
                }
    ...
    def __configuration():
        """Return the configuration of the Divbrowse server"""
        features = {
    ...
    def __genes():
        ...
        r = Response(
            response=ad.genes_list_json_dumped, status=200, mimetype="application/json"
        )
